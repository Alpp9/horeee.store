...
if ( ! class_exists( 'Foxiz_W_Post', false ) ) :
	class Foxiz_W_Post extends WP_Widget {
		...
		function __construct() {
			...
			parent::__construct( $this->widgetID, esc_html__( 'Foxiz - Post Listing', 'foxiz-core' ), array(
		...
		function form( $instance ) {
			...
			foxiz_create_widget_text_field( array(
				...
				'description' => esc_html__( 'Filter posts by multiple category IDs, separated category IDs by commas (for example: 1,2,3).', 'foxiz-core' ),
			...
			foxiz_create_widget_text_field( array(
				...
				'description' => esc_html__( 'Filter posts by tag slugs, separated by commas (for example: tagslug1,tagslug2,tagslug3).', 'foxiz-core' ),
				'value'       => $instance['tags']
			) );
			foxiz_create_widget_text_field( array(
				...
				'description' => esc_html__( 'Exclude tag slugs, separated by commas (for example: tagslug1,tagslug2,tagslug3).', 'foxiz-core' ),
			...
			foxiz_create_widget_text_field( array(
				...
				'description' => esc_html__( 'Exclude posts by Post IDs, separated by commas (for example: 1,2,3).', 'foxiz-core' ),
			...
			foxiz_create_widget_text_field( array(
				...
				'description' => esc_html__( 'Filter posts by post IDs. separated by commas (for example: 1,2,3).', 'foxiz-core' ),
			...
			foxiz_create_widget_text_field( array(
				...
				'description' => esc_html__( 'Input custom entry meta tags to show, separate by comma. For example: avatar,author,update. Keys include: [avatar, author, date, category, tag, view, comment, update, read, custom]', 'foxiz-core' ),
				'value'       => $instance['entry_meta']
			) );
			foxiz_create_widget_select_field( array(
				'id'      => $this->get_field_id( 'featured_position' ),
				'name'    => $this->get_field_name( 'featured_position' ),
				'title'   => esc_html__( 'Featured Position', 'foxiz-core' ),
				'options' => array(
		...
		function widget( $args, $instance ) {
			...
			if ( empty( $instance['entry_meta'] ) ) {
				...
				$instance['entry_meta'] = strval( $instance['entry_meta'] );
			}
