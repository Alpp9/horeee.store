...
if ( ! function_exists( 'foxiz_get_adsense' ) ) {
	function foxiz_get_adsense( $settings ) {
		...
		if ( empty( $settings['code'] ) || foxiz_is_amp() ) {
			return false;
		}
        ...
        <div class="<?php echo esc_attr( $classes ); ?>">
			<?php if ( ! empty( $settings['description'] ) ) : ?>
                <h6 class="ad-description is-meta"><?php echo esc_html( $settings['description'] ); ?></h6>
			<?php endif;
...
if ( ! function_exists( 'foxiz_get_ad_image' ) ):
	function foxiz_get_ad_image( $settings ) {
		...
		if ( ! empty( $settings['description'] ) ) {
			$output .= '<h6 class="ad-description is-meta">' . esc_html( $settings['description'] ) . '</h6>';
		}
		...
		if ( ! empty( $settings['dark_image']['url'] ) ) {
			$output .= '<img data-mode="default" src="' . esc_url( $settings['image']['url'] ) . '" alt="' . esc_attr( $settings['image']['alt'] ) . '"';
			if ( ! empty( $size[0] ) && $size[1] ) {
				$output .= ' width="' . esc_attr( $size[0] ) . '" height="' . esc_attr( $size[1] ) . '"';
			}
			$output .= '>';
			$output .= '<img data-mode="dark" src="' . esc_url( $settings['dark_image']['url'] ) . '" alt="' . esc_attr( $settings['dark_image']['alt'] ) . '"';
			if ( ! empty( $size[0] ) && $size[1] ) {
				$output .= ' width="' . esc_attr( $size[0] ) . '" height="' . esc_attr( $size[1] ) . '"';
			}
			$output .= '>';
		} else {
			$output .= '<img src="' . esc_url( $settings['image']['url'] ) . '" alt="' . esc_attr( $settings['image']['alt'] ) . '"';
			if ( ! empty( $size[0] ) && $size[1] ) {
				$output .= ' width="' . esc_attr( $size[0] ) . '" height="' . esc_attr( $size[1] ) . '"';
			}
			$output .= '>';
...
if ( ! function_exists( 'foxiz_infeed_advert' ) ) {
	/**
	 * @param $settings
	 */
	function foxiz_infeed_advert( $settings ) {
		...
		if ( ! empty( $settings['infeed_ad'] ) ):
            ...
            <div class="<?php echo esc_attr( $class_name ); ?>">
                ...
                elseif ( 'image' === $settings['infeed_ad'] )  :
					$size = foxiz_get_image_size( $settings['ad_attachment'] );
					if ( empty( $settings['ad_destination'] ) ) : ?>
                        <img class="infeed-ad infeed-image" src="<?php echo esc_url( $settings['ad_attachment'] ); ?>" alt="<?php
						if ( ! empty( $settings['infeed_description'] ) ) {
							echo esc_attr( $settings['infeed_description'] );
						} ?>" width="<?php if ( ! empty( $size[0] ) ) {
							echo esc_attr( $size[0] );
						} ?>" height="<?php if ( ! empty( $size[1] ) ) {
							echo esc_attr( $size[1] );
						} ?>">
					<?php else : ?>
                        <a class="infeed-ad infeed-image" target="_blank" href="<?php echo esc_url( $settings['ad_destination'] ); ?>">
                            <img src="<?php echo esc_url( $settings['ad_attachment'] ); ?>" alt="<?php if ( ! empty( $settings['infeed_description'] ) ) {
								echo esc_attr( $settings['infeed_description'] );
							} ?>" width="<?php if ( ! empty( $size[0] ) ) {
								echo esc_attr( $size[0] );
							} ?>" height="<?php if ( ! empty( $size[1] ) ) {
				...
				if ( ! empty( $settings['infeed_description'] ) ) : ?>
                    <div class="ad-description"><?php echo do_shortcode( $settings['infeed_description'] ) ?></div>
				<?php endif; ?>
            </div>
		<?php endif;
	}
