...
if ( ! function_exists( 'foxiz_get_grid_recommended_1' ) ) {
	/**
	 ...
	 * @return false|string
	 */
	function foxiz_get_grid_recommended_1( $settings = array(), $_query = null ) {

		$settings = wp_parse_args( $settings, array(
			'uuid' => '',
			'name' => 'grid_recommended_1'
		) );

		$settings = foxiz_detect_dynamic_query( $settings );

		$settings['classes'] = 'block-grid is-ajax-block block-grid-recommended-1';
		...
		if ( class_exists( 'Elementor\Plugin' ) && \Elementor\Plugin::$instance->editor->is_edit_mode() ) {
			foxiz_live_block_grid_recommended_1( $settings );
		} else {
...
if ( ! function_exists( 'foxiz_loop_grid_recommended_1' ) ) {
	/**
	 * @param  $settings
	 * @param $_query
	 */
	function foxiz_loop_grid_recommended_1( $settings, $_query ) {
...
if ( ! function_exists( 'foxiz_live_block_grid_recommended_1' ) ) {
	/**
	 * @param array $settings
	 *
	 * @return false|string
	 */
	function foxiz_live_block_grid_recommended_1( $settings = array() ) {
