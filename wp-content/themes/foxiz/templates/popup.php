...
if ( ! function_exists( 'foxiz_get_popup_newsletter' ) ) {
	...
	function foxiz_get_popup_newsletter() {
		...
		if ( ! empty( $cover['url'] ) ) {
			...
			$output     .= '<img class="popup-newsletter-img" src="' . esc_url( $cover['url'] ) . '" alt="' . esc_attr( $cover['alt'] ) . '" ';
...
if ( ! function_exists( 'foxiz_render_user_form_popup' ) ) {
	...
	function foxiz_render_user_form_popup() {
		...
		if ( is_user_logged_in() || foxiz_get_option( 'disable_login_popup' ) ) {
			return false;
		}
						...
						<?php if ( ! empty( $settings['header_login_logo']['url'] ) ) : ?>
                            <div class="logo-popup-logo">
								<?php if ( ! empty( $settings['header_login_dark_logo']['url'] ) ) : ?>
                                    <img data-mode="default" src="<?php echo esc_url( $settings['header_login_logo']['url'] ); ?>" alt="<?php echo esc_attr( $settings['header_login_logo']['alt'] ) ?>" height="<?php echo esc_attr( $settings['header_login_logo']['height'] ) ?>" width="<?php echo esc_attr( $settings['header_login_logo']['width'] ) ?>">
                                    <img data-mode="dark" src="<?php echo esc_url( $settings['header_login_dark_logo']['url'] ); ?>" alt="<?php echo esc_attr( $settings['header_login_dark_logo']['alt'] ) ?>" height="<?php echo esc_attr( $settings['header_login_dark_logo']['height'] ) ?>" width="<?php echo esc_attr( $settings['header_login_dark_logo']['width'] ) ?>">
								<?php else : ?>
                                    <img src="<?php echo esc_url( $settings['header_login_logo']['url'] ); ?>" alt="<?php echo esc_attr( $settings['header_login_logo']['alt'] ) ?>" height="<?php echo esc_attr( $settings['header_login_logo']['height'] ) ?>" width="<?php echo esc_attr( $settings['header_login_logo']['width'] ) ?>">
								<?php endif; ?>
                            </div>
						<?php endif;
...
if ( ! function_exists( 'foxiz_render_user_form' ) ) {
	 ...
	 */
	function foxiz_render_user_form( $settings = array() ) {
		...
		$args = array(
			'form_id'      => 'popup-form',
			'label_log_in' => esc_html__( 'Sign In', 'foxiz' ),
		);
...
if ( ! function_exists( 'foxiz_get_adblock_popup' ) ) {
	...
	function foxiz_get_adblock_popup() {
		...
		$output = '<div id="rb-checktag"><div id="rb-adbanner" class="adbanner"><img src="data:image/gif;base64,R0lGODlhAQABAIAAAP///wAAACH5BAEAAAAALAAAAAABAAEAAAICRAEAOw==" alt="adbanner"></div></div>';
...
if ( ! function_exists( 'foxiz_get_footer_slide_up' ) ) {
	...
	function foxiz_get_footer_slide_up() {
		...
		if ( ! foxiz_get_option( 'footer_slide_up' ) || ! foxiz_get_option( 'slide_up_shortcode' ) ) {
			return false;
		}
